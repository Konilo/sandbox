...
In finance, leveraging means obtaining an exposition to an asset that surpasses one's stake. For instance, with a 2&times; (or "2:1") leveraged ETF (LETF), an investment of 1 euro grants 2-euro worth of exposition to the underlying asset of the LETF.
...
- And CL2 also happens to be capitalizing which allows dividends to compound without reduced tax interference and grants the investor full control on their fiscal events.
...
But what is rebalancing exactly? It basically consists is readjusting the leverage factor after it has diverged because of the positive or negative evolution of the price of the asset. Keeping in mind @eq-lev, in the case above where the leverage has decreased at the end of day 2 due to a 2% return (@eq-reb-1), rebalancing would consist in borrowing more which reduces the $equity$ term (@eq-reb-2), in buying more shares without borrowing to increase the $position$ (@eq-reb-3), or in selling the 2% return to bring both the $position$ and the $equity$ back to their initial values (@eq-reb-4).
...
Now let's consider a 2&times; LETF on that index fund with a €100 initial value and daily rebalancing. At the end of day 2, it lost $10 \times 2 = \text{€}20$ and closes at €80. If on the next day, the index returns ~11% which is enough for it to breakeven, this won't be enough for the LETF to breakeven: $\text{€}80 \times (1 + \text{€}10/\text{€}90 \times 2) \approx \text{€}97.78$. To breakeven, the LETF needs the underlying index fund not only to breakeven but to overperfom its intial price with a 12.5% return: $\text{€}80 \times (1 + \text{€}20/\text{€}80) = \text{€}80 \times (1 + \text{€}11.25/\text{€}90 \times 2) = \text{€}80 \times (1 + 0.125 \times 2) = \text{€}100$.

In other words, the return that was enough to bring the index back to €100 (~11%), when multiplied by two (~22%), is not enough to bring back the 2&times; LETF back to €100. That's because shares of the LETF lost more worth than those of the index fund: after the loss on day 2, one percent of a share of the index fund is worth $90 \times 0.01 = 0.9$ cents, while one percent of a share of the 2&times; LETF is only worth $80 \times 0.01 = 0.8$ cents. This is the crux of beta slippage. Note that beta slippage affects all investment, as shown in the anteprevious paragraph, but it is amplified by leverage and volatility as we will explain.

@fig-4 illustrates beta slippage. We have an index fund whose share value oscillates between €102 & €98 constituting a volatile sideways market. The price of a share of the associated 2&times; LETF with daily rebalancing gradually drifts instead staying centered on its intitial value like the index fund. We can also note that, without rebalancing, beta slippage doesn't occur since compounding is a necessary condition for beta slippage.
```{r}
#| label: fig-4
...
And @fig-5 shows that beta slippage is accentuated by the leverage factor.
...
If the daily returns are volatile but positive (or positive and null), beta slippage will cause the daily-rebalanced LETFs to have a return that surpasses the return of the underlying asset multiplied by the leverage (@fig-2 & @fig-6). In such cases, the compounding is in the investor's interest.
```{r}
#| label: fig-6
...
```{r}
#| label: fig-10b
#| fig-cap: "Cumuative return of the MSCI USA index (net, EUR)"
#| fig-pos: "H"
...
```{r}
#| label: fig-10c
#| fig-cap: "Cumuative return of the MSCI USA index (net, EUR) & of the CL2"
#| fig-pos: "H"
...
## Forward simulations
```{r}
plot_letf_expected_returns <- function(
    underlying_return,
    leverage = 2,
...
```{r}
#| label: fig-fwd-10
#| fig-cap: "Expected annual return of a 2x LETF on an underlying asset with an expected return of 10%"
#| fig-pos: "H" 

plot_letf_expected_returns(underlying_return = 0.1)
```

```{r}
#| label: fig-fwd-7
#| fig-cap: "Expected annual return of a 2x LETF on an underlying asset with an expected return of 7%"
#| fig-pos: "H" 

plot_letf_expected_returns(underlying_return = 0.07)
```

```{r}
#| label: fig-fwd-5
#| fig-cap: "Expected annual return of a 2x LETF on an underlying asset with an expected return of 5%"
#| fig-pos: "H" 

plot_letf_expected_returns(underlying_return = 0.05)
```
