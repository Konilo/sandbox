...
plot_leverage_scenario <- function(
    ...
    plot_dt[
        ...
        `:=`(
            ...
            letf_x2_never = index * (index_cumulative_return * 2 + 1),
            letf_x3_never = index * (index_cumulative_return * 3 + 1)
        )
    ]
    plot_dt[
        ,
    ...
    # Create consistent x-axis breaks for both plots
...
# index_timeseries <- upward_stable
# show_series <- c("index", "letf_x2_daily", "letf_x3_daily")
...
@fig-lev-sce-sid-vol, index value goes sideways but with volatility above and below its 100 center point. Don't stay centered on 100, though. This drift is volatility drag.
```{r}
