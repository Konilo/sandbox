...
$$
\lim_{t \to \infty} \text{daily return} = \lim_{t \to \infty} \frac{(1.02)^{t-1} \cdot 0.02}{(1.02)^{t-1} - 0.5}
$$
...
Of course, when the returns are negative, leverage expands the losses (@fig-lev-3). Note that this is not symmetric relative to @fig-lev-2: the LETF leads to less losses than the unbalanced, leveraged investment. This is due to the daily rebalancing: over time, a 2 &times; 2% = 4% daily loss corresponds to a decreasing absolute sum of money lost.
...
I.e. the multplicative factor that was enough to bring the index back to €100 (1.111), when multiplied by two (1.222), is not enough to bring back the 2&times; LETF back to $100. That's because the LETF lost more worth and, after the 10%/20% drop, one percent of the index is worth .9 cents while 1% of the 2&times; LETF is worth only .8 cents.

@fig-lev-4, index value goes sideways but with volatility above and below its 100 center point. Don't stay centered on 100, though. This drift is beta slippage.
```{r}
#| label: fig-lev-4
#| fig-cap: "To name"
#| fig-pos: "H"
...
# plot_leverage_scenario(upward_volatile, c("index", "letf_2x_daily", "invest_2x_never"))
# c("index", "letf_2x_daily", "letf_3x_daily", "invest_2x_never", "invest_3x_never")

sideways_volatile <- c(100, rep(c(102, 98), 8))
plot_leverage_scenario(sideways_volatile, c("index", "letf_2x_daily", "invest_2x_never"))
```

And that's accentuated by the level of leverage
