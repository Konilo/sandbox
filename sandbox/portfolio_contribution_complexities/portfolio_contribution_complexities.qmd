...
When contributing to an investment portfolio that follows a strategy involving more than one asset -- each with its own target allocation -- while ruling out selling, computing the amount to contribute to each asset can be tricky. This study explains that challenge and provides its solution.
...
In that case, 400 € of the 1000 € investment are allocated to the index ETF and 600 € to the managed-futures ETF to reach the targets. The math is simple, no selling is required to stay balanced.
...
When selling is allowed, each $\Delta_i$ is just the asset's post-contribution target value minus its pre-contribution value (which can be negative when selling is required):
...
Reusing the portfolio from the pre-investment situation above:
