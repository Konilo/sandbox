...
dt_long <- melt(
    dt,
    id.vars = "age",
    measure.vars = c("sp", "cvfs"),
    variable.name = "component",
    value.name = "amount"
)

ggplot(data = dt_long, aes(x = age, y = amount, fill = component)) +
    geom_bar(stat = "identity") +
    scale_fill_manual(
        name = "Portfolio Components",
        values = c("sp" = "#1f77b4", "cvfs" = "#ff7f0e"),
        labels = c("sp" = "Savings Portfolio", "cvfs" = "Current Value of Future Savings")
    ) +
    ...
    labs(
        title = "Total Portfolio Over Life",
        x = "Age",
        y = "Portfolio Value"
    ) +
    theme_minimal() +
...
ggplot(data = dt, aes(x = age)) +
    geom_line(
        aes(y = stocks_share_tp_appl_sp * 100, color = "Savings Portfolio"),
        linewidth = 1
    ) +
    geom_line(
        aes(y = stocks_share_tp_appl_tp * 100, color = "Total Portfolio"),
        linewidth = 1
    ) +
    scale_color_manual(
        name = "Allocation Method",
        values = c("Savings Portfolio" = "#1f77b4", "Total Portfolio" = "#ff7f0e")
    ) +
    labs(
        title = "Asset Class Allocation Over Life",
        x = "Age",
        y = "Stocks Share (%)"
    ) +
    theme_minimal() +
...
temp_div_dt_long <- melt(
    temp_div_dt,
    id.vars = "age_bin",
    measure.vars = c(
        "share_tot_stocks_euro_months_appl_sp",
        "share_tot_stocks_euro_months_appl_tp",
        "ideal_share_tot_stocks_euro_months"
    ),
    variable.name = "method",
    value.name = "share"
)

temp_div_dt_long[, method := factor(
    method,
    levels = c(
        "share_tot_stocks_euro_months_appl_sp",
        "share_tot_stocks_euro_months_appl_tp",
    ...
    labels = c(
        "Merton-Samuelson Share Applied to Savings Portfolio",
        "Merton-Samuelson Share Applied to Total Portfolio",
        "Perfect Temporal Diversification"
    )
)]

ggplot(data = temp_div_dt_long, aes(x = age_bin, y = share * 100, fill = method)) +
    geom_bar(stat = "identity", position = "dodge") +
    scale_fill_manual(
        name = "Method",
        values = c(
            "Merton-Samuelson Share Applied to Savings Portfolio" = "#1f77b4",
            "Merton-Samuelson Share Applied to Total Portfolio" = "#ff7f0e",
            "Perfect Temporal Diversification" = "#2ca02c"
        )
    ) +
    labs(
        title = "Exposition to Stocks Over Life",
        x = paste0("Age (", years_step, "-year bins)"),
        y = "Share of Working Life Stocks Euro-months (%)"
    ) +
    theme_minimal() +
    theme(
        legend.position = "bottom",
