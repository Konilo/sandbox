...
plot_leverage_scenario <- function(index_timeseries) {
    ...
    plot_dt[
        ,
        `:=`(
            letf_x2_daily_return = letf_x2 / shift(letf_x2, fill = init_value) - 1,
            letf_x3_daily_return = letf_x3 / shift(letf_x3, fill = init_value) - 1
        )
    ]
    ...
    prices <- ggplot(plot_dt, aes(x = trading_day)) +
        ...
        geom_point(aes(y = letf_x2, color = "x2 LETF")) +
        geom_point(aes(y = letf_x3, color = "x3 LETF")) +
        scale_color_manual(
            values = c(
                "Index" = "#1f77b4",
                "x2 LETF" = "#ff7f0e",
        ...
        scale_x_continuous(
            breaks = seq(plot_dt[, min(trading_day)], plot_dt[, max(trading_day)])
        ) +
        labs(
            title = "Prices",
            x = element_blank(),
    ...
    returns <- plot_dt |>
        melt(
            id.vars = "trading_day",
            measure.vars = c("index_daily_return", "letf_x2_daily_return", "letf_x3_daily_return"),
            variable.name = "asset",
            value.name = "daily_return"
        ) |>
        ggplot(aes(x = trading_day, y = daily_return * 100, fill = asset)) +
        geom_bar(stat = "identity", position = position_dodge(width = 0.8)) +
        scale_fill_manual(
            values = c(
                "index_daily_return" = "#1f77b4",
                "letf_x2_daily_return" = "#ff7f0e",
                "letf_x3_daily_return" = "#ff0e0eff"
            ),
            labels = c(
                "index_daily_return" = "Index",
                "letf_x2_daily_return" = "x2 LETF",
                "letf_x3_daily_return" = "x3 LETF"
            )
        ) +
        scale_x_continuous(
            breaks = seq(plot_dt[, min(trading_day)], plot_dt[, max(trading_day)])
...
In such a case with low volatility, the performance of LETFs strictly equals the performance of the index times the amount of leverage. When that trend goes upward, long LETFs (i.e., LETFs whose leverage is positive) are very efficient in terms risk-adjusted yield **can I say that?**.
