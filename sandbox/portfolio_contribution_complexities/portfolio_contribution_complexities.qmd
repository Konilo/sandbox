...
- Hence, a balanced portfolio is characterized by blocks (assets) of equal heights.
...
- That water (contribution) recursively flows on top of any lower block (asset): it acts like real water would and exactly matches the algorithm described in @sec-method.
- Any block (asset) that ends up without water (contribution) above itself is stuck: that's what the red highlighting shows.
- The contribution each asset is granted corresponds to the $\Delta$ euro amount represented as the surface of water on top of each block.
- After pouring water (contributing), the portfolio reached balance if the combined block height and water height is identical for all assets.
...
plot_water_filling <- function(
    ...
    # Solve the no-selling allocation: find the stuck set, h*, and Deltas.
    stuck <- rep(FALSE, n)
    repeat {
    ...
    delta <- ifelse(stuck, 0, (h_star - heights) * tw)
    ...
    ggplot(d) +
        ...
        # in-water-column Delta labels
        geom_text(
            data = d[stuck == FALSE],
            aes(
                x = (xmin + xmax) / 2,
                y = (pedestal_top + h_star) / 2,
                label = paste0("Δ = ", vapply(delta, fmt_num, ""), " €")
            ),
...
```{r}
#| label: fig-demo-case-1
#| fig-cap: "Demo case 1: no recursion (no asset is stuck). Cf. [demo case 1 in section 6.4.1](#demo-case-1) for the algorithm demo."
...
```{r}
#| label: fig-demo-case-2
#| fig-cap: "Demo case 2: single recursion (Index ETF stuck). Cf. [demo case 2 in section 6.4.1](#demo-case-2) for the algorithm demo."
...
```{r}
#| label: fig-demo-case-3
#| fig-cap: "Demo case 3: two recursions (assets 1 and 2 stuck). Cf. [demo case 3 in section 6.4.1](#demo-case-3) for the algorithm demo."
...
```{r}
#| label: fig-demo-case-4
#| fig-cap: "Demo case 4: one stuck asset & two non-stuck assets getting different $\\Delta$s. Cf. [demo case 4 in section 6.4.1](#demo-case-4) for the algorithm demo."
...
1. []{#demo-case-1} **No recursion needed:** reusing the portfolio from @sec-no-selling-needed (cf. @fig-demo-case-1 for the physical model demo):
...
2. []{#demo-case-2} **Single recursion step:** the portfolio from @sec-selling-allowed, but applying the no-selling rule from @sec-deficits-negative -- the index ETF gets stuck and all cash goes to the managed-futures ETF (cf. @fig-demo-case-2 for the physical model demo):
...
3. []{#demo-case-3} **Two recursion steps:** a 3-asset portfolio where one asset is initially overweight, and dropping it reveals a second overweight asset via renormalization, running with `verbose = TRUE` to trace the iterations (cf. @fig-demo-case-3 for the physical model demo):
...
4. []{#demo-case-4} **One recursion step and >1 non-stuck asset**: a 3-asset portfolio where one asset is stuck while the two non-stuck ones get different $\Delta$s (cf. @fig-demo-case-4 for the physical model demo):
